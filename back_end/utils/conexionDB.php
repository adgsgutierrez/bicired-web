...
class ConexionDB {
  ...
  private static function createTables(){
    ...
    $sql_2 = "create TABLE TBL_PUBLICACION( pk_pbl_id int unsigned primary key, pbl_fecha datetime not null, pbl_ltd_origen double not null, pbl_ltg_origen double not null, pbl_ltd_destino double not null, pbl_ltg_destino double not null, pbl_descripcion text, pbl_estado ENUM('A', 'S'), fk_pbl_usr_correo varchar(150), FOREIGN KEY (fk_pbl_usr_correo) references TBL_USUARIO (pk_usr_correo) )";
    $sql_3 = "create TABLE TBL_AMIGOS ( fk_amg_origen varchar(150), fk_amg_destino varchar(150), amg_fecha date, amg_estado date, FOREIGN KEY (fk_amg_origen) references TBL_USUARIO (pk_usr_correo), FOREIGN KEY (fk_amg_destino) references TBL_USUARIO (pk_usr_correo) )";
